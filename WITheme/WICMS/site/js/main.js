...
// WICMS clean-base guard: older site templates may call optional theme plugins
// before those plugin files exist. Missing optional plugins must not break login
// pages or the base public shell.
(function (jq) {
	if (!jq || !jq.fn) { return; }
	var noop = function () { return this; };
	var plugins = [
		'owlCarousel', 'parallax', 'bxSlider', 'jflickrfeed', 'tweetable',
		'timeago', 'masonry', 'isotope', 'magnificPopup', 'affix',
		'jogjaContact', 'tooltip'
	];
	for (var i = 0; i < plugins.length; i++) {
		if (typeof jq.fn[plugins[i]] !== 'function') {
			jq.fn[plugins[i]] = noop;
		}
	}
	if (typeof window.Swipe !== 'function') {
		window.Swipe = function () { return {}; };
	}
}(jQuery));
$(document).ready(function($) {

	var searchEl = document.getElementById('sb-search');
	if (searchEl && typeof window.UISearch === 'function') {
		new window.UISearch(searchEl);
	}


	"use strict";
		if (jQuery('.fullwidthbanner').length && typeof jQuery.fn.revolution === 'function') {
