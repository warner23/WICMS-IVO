 ...
 * File: WIEventsService.php
 * Location: /WIAdmin/WIPlugin/WIEvents/WIAdmin/WICore/WIClass/WIEventsService.php
 * Type: Plugin class
 * Layer: Service / Business logic
 * Purpose Area: WIEvents workspace and record orchestration
 * Version: 0.1.0-dev
 * Created: 2026-06-01
...
final class WIEventsService
{
    private WIEventsRepository $repository;

    public function __construct(WIEventsRepository $repository)
    {
    ...
    {
        return $this->success('WIEvents workspace loaded.', [
            'plugin' => [
                'code' => 'wievents',
                'name' => 'WIEvents',
                'area' => 'hospitality_operations',
                'description' => 'Event operations plugin for private dining, functions, weddings, conferences and outside catering events.',
            ],
            ...
            'record_types' => ["event", "function_sheet", "event_menu", "event_task"],
            'states' => ['grey', 'green', 'amber', 'red'],
            'statuses' => ['open', 'in_progress', 'complete', 'cancelled', 'archived'],
            'priorities' => ['low', 'normal', 'high', 'critical'],
            'plugin_boundaries' => [
                'owns' => ["event", "function_sheet", "event_menu", "event_task"],
                'optional_integrations' => ["WIRestaurant", "WIRooms", "WISpecs", "WITaskEngine", "WIProfitEngine"],
    ...
    public function updateState(array $request = []): array
    {
        ...
        $this->repository->logEvent([
            ...
            'related_id' => $id,
            'state' => $state,
            'message' => 'WIEvents state changed to ' . $state . '.',
            'created_by_user_id' => $request['user_id'] ?? null,
        ]);
