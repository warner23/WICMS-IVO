...
WIAdmin.displayInfo = function (userId) {
jQuery.noConflict();
   ...
   //display modal
   //modal.modal('show');
   $("#modal-user-details").removeClass('hide');
 $("#modal-user-details").addClass('show');
 ...
 */
WIAdmin.deleteUser = function (element, userId) {
 ...
 */
WIAdmin.changeRole = function (element, role, userId) {
  //send data to server
    $.ajax({
        type: "POST",
        url: "WICore/WIClass/WIAjax.php",
        ...
        success: function (newRole) {
            ...
            element.text(newRole);
             $("#modal-change-role").removeClass('show').addClass('hide');
...
WIAdmin.roleChanger = function (element, userId) {
    jQuery.noConflict();
    ...
    */
       $("#modal-change-role").removeClass('hide').addClass('show');
   ...
   //find elements needed for changing text
    ...
    $("#change-role-button").unbind().bind('click', function () {
        ...
        WIAdmin.changeRole(userRoleSpan, newRole, userId);
    });
};


WIAdmin.showAddUserModal = function () {
    jQuery.noConflict();
    ...
    WIAdmin.addEditData.button.attr('onclick', 'users.addUser();');
};


WIAdmin.addUser = function (editMode) {
    var data  = WIAdmin.addEditData;
    ...
    if ( WIAdmin._validateUser(data, forUpdate) ) {
...
WIAdmin.updateUser = function () {
    WIAdmin.addUser(true);
};

WIAdmin.editUser = function (userId) {
    $("#adduser-userId").val(userId);

    var modal = $("#modal-add-edit-user"),
        data  = WIAdmin.addEditData,
        modalTitle = modal.find(".modal-title"),
        modalBody  = modal.find(".modal-body"),
        ajaxLoading = modal.find(".ajax-loading");

    data.button.attr('onclick', 'WIAdmin.updateUser();');
    ...
    $.ajax({
        type: "POST",
        url: "WICore/WIClass/WIAjax.php",
        data: { action : "getUser", userId : user_Id },
        success : function (result) {
            //parse result to JSON
            var res = JSON.parse(result);
...
WIAdmin.banUser = function (el, userid) {
    $.ajax({
        type: "POST",
        url: "WICore/WIClass/WIAjax.php",
        data: {
...
WIAdmin.unbanUser = function (el, userid) {
    $.ajax({
        type: "POST",
        url: "WICore/WIClass/WIAjax.php",
        data: {
...
WIAdmin._validateUser = function (data, forUpdate) {
    var valid = true;
    ...
    if ( $.trim( data.username.val() ) == "" ) {
        valid = false;
        WICore.displayErrorMessage( data.username, $_lang.field_required );
    }
    ...
    if ( forUpdate === true )
    {
        if ( data.password.val().length > 0 && data.password.val().length < 6 ) {
            valid = false;
            WICore.displayErrorMessage( data.password, $_lang.password_length );
        }

        if ( data.password.val() != data.conf_password.val() ) {
            valid = false;
            WICore.displayErrorMessage( data.password );
            WICore.displayErrorMessage( data.conf_password, $_lang.passwords_dont_match );
        }
    }
    else
    {
        if ( $.trim( data.password.val() ) == "" ) {
            valid = false;
            WICore.displayErrorMessage( data.password, $_lang.field_required );
        }
        ...
        if ( $.trim( data.conf_password.val() ) == "" ) {
            valid = false;
            WICore.displayErrorMessage( data.conf_password, $_lang.field_required );
        }
