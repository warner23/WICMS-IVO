...
class template
{
    ...
    {
        $page = 'template';
        $heading = $this->escape($this->moduleText($page, 'text', 'WICMS Template'));
        $body = $this->escape($this->moduleText($page, 'text1', 'Generic public template page.'));
        echo <<<HTML
...
HTML;
    ...
    {
        $this->Boot->startMod($page, 'WICMS Template', '<p>Generic public template page.</p>');
        ...
        echo '<h1 style="margin:0 0 12px;color:#0f172a;">' . $this->escape('WICMS Template') . '</h1>';
        echo '<p style="margin:0;color:#475569;font-size:16px;line-height:1.7;">' . $this->escape('This template is clean and plugin-neutral so future themes can safely replace it.') . '</p>';
        echo '</section>';
