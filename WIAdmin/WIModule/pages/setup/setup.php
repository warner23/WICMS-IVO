...
class setup
{
    ...
    {
        $page = 'setup';
        $heading = $this->escape($this->moduleText($page, 'text', 'Setup'));
        $body = $this->escape($this->moduleText($page, 'text1', 'Core setup page placeholder.'));
        echo <<<HTML
...
HTML;
    ...
    {
        $this->Boot->startMod($page, 'Setup', '<p>Core setup page placeholder.</p>');
        ...
        echo '<h1 style="margin:0 0 12px;color:#0f172a;">' . $this->escape('Setup') . '</h1>';
        echo '<p style="margin:0;color:#475569;font-size:16px;line-height:1.7;">' . $this->escape('Use this clean setup area for WICMS configuration. Industry setup wizards belong in plugin packages.') . '</p>';
        echo '</section>';
