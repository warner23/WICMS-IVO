...
class training
{
    ...
    {
        $page = 'training';
        $heading = $this->escape($this->moduleText($page, 'text', 'Training'));
        $body = $this->escape($this->moduleText($page, 'text1', 'Core training page placeholder.'));
        echo <<<HTML
...
HTML;
    ...
    {
        $this->Boot->startMod($page, 'Training', '<p>Core training page placeholder.</p>');
        ...
        echo '<h1 style="margin:0 0 12px;color:#0f172a;">' . $this->escape('Training') . '</h1>';
        echo '<p style="margin:0;color:#475569;font-size:16px;line-height:1.7;">' . $this->escape('Training modules can be installed later as plugins or packages.') . '</p>';
        echo '</section>';
