...
class users
{
    ...
    {
        $page = 'users';
        $heading = $this->escape($this->moduleText($page, 'text', 'Users'));
        $body = $this->escape($this->moduleText($page, 'text1', 'Core users page placeholder.'));
        echo <<<HTML
...
HTML;
    ...
    {
        $this->Boot->startMod($page, 'Users', '<p>Core users page placeholder.</p>');
        ...
        echo '<h1 style="margin:0 0 12px;color:#0f172a;">' . $this->escape('Users') . '</h1>';
        echo '<p style="margin:0;color:#475569;font-size:16px;line-height:1.7;">' . $this->escape('User management belongs to WICMS core; advanced HR or compliance assignment flows should be installed separately.') . '</p>';
        echo '</section>';
