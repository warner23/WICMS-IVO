...
class WILogin
{
    ...
    public function StaffLogin($username, $password) {
        ...
        if(count($result) == 1) 
        {
            ...
            //user exist, log him in if he is confirmed
            WISession::set("user_id", $result[0]['user_id']);
             WISession::set("role_id", $result[0]['user_role']);
            session_regenerate_id(true);
            if(LOGIN_FINGERPRINT == true)
                WISession::set("login_fingerprint", $this->_generateLoginString ());
            //echo $result[0]['user_role'] .'role';
            $granted = $this->perm->hasPermission($result[0]['user_role']);
            //echo $granted;
            //return true;
          ...
          echo json_encode(array(
            ...
            $st1  = $username ;
            $st2  = "Successfully logged in  compliance Staff:  . ' $username . '" ;
            //$maintain = new WIMaintenace();
            $this->maint->LogFunction($st1, $st2);
        //echo "log in" . WISession::get("role_id");
