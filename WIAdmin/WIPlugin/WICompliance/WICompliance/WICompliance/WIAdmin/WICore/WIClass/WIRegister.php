...
class WIRegister  
{
    ...
    public function registerStaff($data) 
    {
        ...
        if(count($errors) == 0) {
           ...
           $this->WIdb->insert('wi_compliance_staff', array(
            "staff_email"     => $user['email'],
            "staff_name"  => strip_tags($user['username']),
            "staff_password"  => $this->hashPassword($user['password']),
            "staff_site" => $user['site'],
...
//            echo " my total" . $user['bot_sum'];
    ...
    public function validateStaff($data, $botProtection = true)
    {
        $id     = $data['FieldId'];
        $user   = $data['UserData'];
        $errors = array();
        ...
        //check if email is available
        if( $validator->StaffEmailExist($user['email']) )
            $errors[] = array( 
                "id"    => $id['email'],
                "msg"   => WILang::get('email_taken')
            );
        ...
        //check if username is available
        if( $validator->staffnameExist($user['username']) )
            $errors[] = array( 
                "id"    => $id['username'],
                "msg"   => WILang::get('username_taken')
            );
