...
class WIEmail {
    ...
    function confirmationEmail($email, $key) 
    {
        ...
        function confirmationAppointmentEmail($email, $docReceipt) 
    {
        // get instance of PHPMailer (including some additional info)
        $mail = $this->_getMailer();
    ...
    function passwordResetEmail($email, $key) 
    {
        ...
        $link = '<a href="' .WEBSITE_DOMAIN.''.REGISTER_PASSWORD_RESET . '?k=' . $key.'">Reset Password</a>';


        $body = file_get_contents(dirname(dirname(__FILE__)) . '/WITemp/forgot-password-mail.php');

        $body = str_replace('{{website_name}}',WEBSITE_NAME, $body);
        $body = str_replace('{{linkage}}',$link, $body);
    ...
    function contactEmail($email, $name, $subject, $message)
    {
        // get instance of PHPMailer
        $mail = $this->_getMailer();

        // where you want to send the email
        $endAddress = CONTACT_EMAIL;
        $mail->addAddress($endAddress);

         $body = file_get_contents(dirname(dirname(__FILE__)) . '/WITemp/contact-us.php');
         $body = str_replace('{{name}}',$name, $body);
         $body = str_replace('{{subject}}',$subject, $body);
         $body = str_replace('{{message}}',$message, $body);
         $body = str_replace('{{from}}',$email, $body);
