...
class WILogin
{
    ...
    public function StaffLogin($username, $password) {
        ...
        $result = $this->WIdb->select(
                    "SELECT * FROM `wi_compliance_staff`
                     WHERE `staff_email` = :u AND `staff_password` = :p",
                     array(
                       "u" => $username,
                       "p" => $password
                     )
        ...
        if(count($result) == 1) 
        {
            ...
            //user exist, log him in if he is confirmed
            WISession::set("staff_id", $result[0]['staff_id']);
            session_regenerate_id(true);
            if(LOGIN_FINGERPRINT == true)
                WISession::set("login_fingerprint", $this->_generateLoginString ());
            ...
            $st1  = $username ;
            $st2  = "Successfully logged in Staff:  . ' $username . '" ;
            //$maintain = new WIMaintenace();
            $this->maint->LogFunction($st1, $st2);
        echo "log in" . WISession::get("staff_id");
