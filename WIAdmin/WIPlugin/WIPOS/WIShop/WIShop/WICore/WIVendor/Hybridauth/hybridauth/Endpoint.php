...
class Hybrid_Endpoint {
	...
	protected function processOpenidXRDS() {
		...
		$output = str_replace("{RETURN_TO_URL}", str_replace(
						array("<", ">", "\"", "'", "&"), array("&lt;", "&gt;", "&quot;", "&apos;", "&amp;"), Hybridauth::getCurrentUrl(false)
	...
	protected function processOpenidRealm() {
		...
		$output = str_replace("{X_XRDS_LOCATION}", htmlentities($this->hybrid->getCurrentUrl(false), ENT_QUOTES, 'UTF-8')
				. "?get=openid_xrds&v="
				. $this->hybrid->$version, file_get_contents(dirname(__FILE__) . "/resources/openid_realm.html"));
		print $output;
		die();
	}
	...
	protected function processAuthStart() {
		...
		if (!Hybridauth::storage()->get("hauth_session.$provider_id.hauth_endpoint")) {
	...
	protected function processAuthDone() {
		...
		$hauth = Hybridauth::setup($provider_id);
