...
class WIRole {
    ...
    public function getId($name) {
        $result = $this->WIdb1->select("SELECT `role_id` FROM `wi_user_roles` WHERE `role` = :r", array( 'r' => $name ));
        if ( count ( $result ) > 0 )
    ...
    public function name($id) {
        $result = $this->WIdb1->select("SELECT `role` FROM `wi_user_roles` WHERE `role_id` = :id", array( 'id' => $id ));
        if ( count ( $result ) > 0 )
            return $result[0]['role_id'];
        else
            return null;
