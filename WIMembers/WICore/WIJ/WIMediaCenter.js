...
function sendFileToAvatarServer(formData,status)
{
      var uploadURL ="WICore/WIClass/AvatarImageUpload.php"; //Upload URL
    var extraData ={}; //Extra Data.
    var jqXHR=$.ajax({
            xhr: function() {
            var xhrobj = $.ajaxSettings.xhr();
    ...
    processData: false,
        ...
        success: function(data){
            ...
            $("#dragandrophandler").remove();
            //$("img.cp").remove();
            var preview = '<img src="'+info.name+'" class="img-responsive cp" style="width:140px;" id="avatarPic" value="'+info.id+'">';
            $("#preview").empty();
...
function handleFileUpload(files,obj,dir)
{
   for (var i = 0; i < files.length; i++) 
   {
        ...
        if (dir === "header") {
            ...
            sendFileToAvatarServer(fd,status);
        }
